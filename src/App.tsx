...
class Point {
  ...
  constructor({ x, y, originX, originY }: { x: number; y: number; originX?: number; originY?: number }) {
    this.x = x;
    this.y = y;
  ...
  useEffect(() => {
    ...
    const drawLines = (p: Point) => {
      for (let i = 0; i < points.current.length; i++) {
        ...
        // Show lines only when the point is near the target (mouse position)
  ...
  return (
    <BrowserRouter>
      <div className="bg-primary relative z-0">
        <div id="large-header" className="bg-cover bg-center bg-no-repeat large-header" ref={largeHeaderRef}>
          <Navbar />
